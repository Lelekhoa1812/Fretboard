...
export class MusicTheoryAI {
  ...
  static async analyzeChordProgression(chords, sessionId = null) {
    const prompt = `You are analyzing this chord progression on a virtual guitar fretboard: **${chords.join(' - ')}**
    ...
    **Provide a comprehensive analysis using markdown formatting:**
    ...
    - Difficulty level for guitar players

    ## 🎸 **Fretboard Guidance**
    - Specific fret positions for each chord
    - Recommended fingerings and techniques
    - Smooth transition tips between chords

    ## 🎼 **Music Theory Deep Dive**
    - Harmonic function of each chord (Tonic, Subdominant, Dominant)
    - Voice leading suggestions for smooth movement
    - Scale relationships and improvisation opportunities

    ## 🔄 **Chord Progressions & Variations**
    - Alternative chord substitutions
    - Common progressions this resembles
    - Extended chord options (7ths, 9ths, etc.)
    - Inversions and slash chords

    ## 💡 **Creative Applications**
    - Strumming patterns that work well
    - Fingerpicking possibilities
    - Songwriting suggestions
    - Genres this progression fits

    ## 🎯 **Next Steps**
    - Practice exercises to master this progression
    - Related scales to explore
    - Songs that use similar progressions

    Be specific, encouraging, and focus on practical guitar applications that enhance the user's fretboard experience.`;
